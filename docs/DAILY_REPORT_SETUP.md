...
- **今日完成**：按“应用 · 主要工作”分组，逐条说明对象、动作和当前结果；仅有窗口记录时不会被表述成已完成成果。
- **时间投入**：每个“应用 · 主要工作”单列时长、占比和原生文本进度条，便于比较投入而不需要图片渲染。
- **工作节奏**：有效活动时长/占比、最长连续工作段、应用与窗口切换次数、键盘和鼠标输入等可观测指标；不把这些信号解释为心理状态。
...
飞书公开卡片能力当前没有稳定通用的服务端图表组件可供此场景依赖，因此时间和节奏数据使用原生分栏、指标块和进度条文本呈现。这样既不依赖图片生成，也不会因图表上传失败导致日报发送失败。
