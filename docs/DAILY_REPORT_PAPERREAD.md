...
文档变更部分读取 Windows 长驻事件监听器写入的 SQLite 事件队列，并筛选当天 01:00 到运行时刻的新增、编辑、标题修改、回收和删除事件。它只描述事件元数据，不能从事件元数据恢复具体被删除或修改的句子。事件监听器使用飞书官方 SDK 长连接运行；如果日报显示“未收到事件”，不能据此确认当天没有变更，应检查监听器日志、事件队列路径、应用版本中的云文档事件订阅，以及日报机器人对目标知识库/文档的应用身份权限。
